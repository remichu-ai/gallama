...
def mimo_tool_parser(tool_text: str, extra_vars: dict = None) -> List[ParsedToolCall]:
    """
    Parse MiMo V2 XML-style tool call format:

    <tool_call>
    <function=tool_name>
    <parameter=param_key_1>param_value_1</parameter>
    </function>
    </tool_call>
    """
    ...
    try:
        for func_match in function_pattern.finditer(tool_text):
            ...
            arguments_dict = {}
            for param_match in parameter_pattern.finditer(func_body):
...
mimo = {
    "tool": TagDefinition(
        ...
        api_tag="tool_calls",
        role="assistant",
        post_processor=mimo_tool_parser,
