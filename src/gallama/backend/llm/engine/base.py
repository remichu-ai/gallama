...
class ModelInterface(ABC):
    ...
    def __init__(self, model_spec:ModelSpec):
        ...
        if model_spec.draft_model_id:
            ...
            if not draft_model_config:
                raise ModelError(f"Model config for '{model_spec.draft_model_name}' not exist")
    ...
    ## ******************************************************************************
    async def chat(self, query: ChatMLQuery, gen_queue: GenQueue, request: Request = None):
        """
    ...
    def validate_token_length(self, token_length):
        ...
        if self.max_seq_len and token_length > self.max_seq_len:
            raise ModelError(status_code=400, detail=f"Token length exceeds max length of {self.max_seq_len}")
    ...
    async def chat_no_tool(self, query: ChatMLQuery, prompt_eng: PromptEngine, gen_queue, request: Request):
        ...
        if query.thinking_template:
            try:
                ...
                raise ModelError(status_code=400, detail=f"thinking_template is not valid XML string")
